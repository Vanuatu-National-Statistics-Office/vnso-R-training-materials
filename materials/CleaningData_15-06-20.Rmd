...
motorCarTrends <- read.xlsx("data", "carInfo_27-05-20.xlsx", header= TRUE)
```
...
motorCarPrices<- read.csv("data", "carPrices_27-05-20.csv", header = TRUE) # motorCarTrends has already been read in
...
# Replace multiple values with NA value (parameters x, y)
mergedMotorCarTrendsWithoutDuplicates$HorsePower[mergedMotorCarTrendsWithoutDuplicates$HorsePower %in% c(x,y)] <- NA
``` 
...
mergedMotorCarTrendsWithoutDuplicates$MilesPerUSD <- mergedMotorCarTrendsWithoutDuplicates$MilesPerUSD / 10 #cost of gallon petrol is $10
```
